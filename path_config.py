...
class Path(luigi.Config):
    ...
    ground_refinement_dir = (
        "/dls/science/groups/i04-1/elliot-dev/Work/"
        "exhaustive_parse_xchem_db/ground_refinement"
    )

    # Csvs from parsing the database
    log_pdb_mtz = luigi.Parameter(default=os.path.join(out_dir, "log_pdb_mtz.csv"))
    log_occ = luigi.Parameter(default=os.path.join(out_dir, "log_occ.csv"))
    log_occ_resname = luigi.Parameter(
        default=os.path.join(out_dir, "log_occ_resname.csv")
    )

    occ_conv = luigi.Parameter(default=os.path.join(out_dir, "occ_conv.csv"))
    refinement_summary = luigi.Parameter(
        default=os.path.join(out_dir, "refinement_summary.csv")
    )
    refine = luigi.Parameter(default=os.path.join(out_dir, "refinement.csv"))
    superposed = luigi.Parameter(default=os.path.join(out_dir, "superposed.csv"))
